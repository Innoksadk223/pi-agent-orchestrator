...
export default function agentOrchestrator(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use agent_team plan for complex Loop work: register the complete fixed roster, reviewerId, execution DAG, concrete cwd-relative owned paths, local TaskPackets, and acceptance. With auto off this is one USER_GATE; set-auto may authorize initial plans and amendments for only the current runtime session. Amend only with the current expectedRevision.",
